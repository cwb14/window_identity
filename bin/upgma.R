...
# Computes divergence time based on genetic distance and mutation rate
compute_divergence_time <- function(D, mutation_rate) {
    time_matrix <- D / (2 * mutation_rate)
...
plot_tree_ape <- function(newick_file, output_pdf, name_key=NULL, tree_type="divergence", mutation_rate=NULL) {
    ...
    x_axis_label <- if (tree_type == "time") "Time (years)" else "Divergence K"
...
option_list = list(
    ...
    make_option(c("--mutation_rate"), type="numeric", default=1.3e-8, 
                help="mutation rate for divergence time calculation", metavar="numeric"),
...
# Process all cleaned_matrix*.tsv files in the directory
files <- list.files(pattern="cleaned_matrix.*\\.tsv")
...
for (file in files) {
    ...
    # Save the raw Newick tree
    output_file <- sub(".tsv", ".nwk", file)
 #   newick_tree <- gsub("e-?[0-9]+", "0", newick_tree)  # Replace scientific notation with "0". Doesnt work. Also, it wasnt the source of the issue. 
    ...
    # Plot time-based tree and save as ape PDF
    time_pdf <- sub(".nwk", ".time.nwk.ape.pdf", time_output_file)
